((): void => {
    ...
    const convertNumber = (value: number): void => {
        console.log(value);
        const table: HTMLElement | null = document.getElementById("output");
    ...
    window.onload = (): void => {
        ...
        const dropdown = document.querySelector("select");
        // @ts-ignore
        dropdown.onchange = (event): void => convertNumber(Number(event.target.value));
    };
})();
